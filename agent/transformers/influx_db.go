...
func splitWithoutEscaped(str string, delim string) []string {
	escapedString := regexp.MustCompile(`\\` + delim)
	str = escapedString.ReplaceAllString(str, "\x00")
	segments := strings.Split(str, delim)
	for i := range segments {
		segments[i] = strings.ReplaceAll(segments[i], "\x00", delim)
	}
	return segments
}
